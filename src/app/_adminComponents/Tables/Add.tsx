...
const initialValues: TableType = {
    tableNumber: "",
};
const validationSchema = Yup.object({
    tableNumber: Yup.number().positive().min(1).required("رقم الترابيزة مطلوب"),
});

export default function AddTable() {
    const router = useRouter();
    const [loading, setLoading] = useState<boolean>(false);
    const formik = useFormik<TableType>({
        initialValues,
        validationSchema,
        onSubmit: async (values) => {
            setLoading(true)
            try {
                const { status } = await axios.post(`${hostUrl}/table`, values, { headers })
                if (status === 201) {
                    setLoading(false)
                    toast.success("تم إضافة ترابيزة بنجاح", {
                        duration: 4000,
                        style: { width: "100%", marginTop: "30px" },
                    });
                    router.push("/dashboard/tables-managment")
                }
            } catch (error: any) {
                setLoading(false)
