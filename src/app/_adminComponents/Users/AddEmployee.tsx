...
const validationSchema = Yup.object({
    email: Yup.string().required("الإيميل مطلوب"),
});
...
export default function AddEmployee() {
    ...
    const formik = useFormik<EmployeeType>({
        ...
        onSubmit: async (values) => {
            setLoading(true)
            try {
                const { status } = await axios.patch(`${hostUrl}/auth/add/new-employee`, values, { headers })
                if (status === 200) {
                    setLoading(false)
                    toast.success("تم تغير الدور لموظف بنجاح", {
                        duration: 4000,
                        style: { width: "100%", marginTop: "30px" },
                    });
                    router.push("/dashboard/users-managment")
                }
            } catch (error: any) {
                setLoading(false)
